...
import bagels.brands.gradle.isRootProject
import bagels.brands.gradle.listProperty
import bagels.brands.gradle.properties.ProjectProperties
import bagels.brands.gradle.properties.mapToInt
import bagels.brands.gradle.property
import bagels.brands.gradle.register
import bagels.brands.gradle.tasks.BootstrapUtils.computeDaemonArgs
import bagels.brands.gradle.util.isCi
import bagels.brands.gradle.util.jdkVersion
...
private val bytesPerGB = 1024.0.pow(3)
...
constructor(objects: ObjectFactory, providers: ProviderFactory) : DefaultTask() {
    ...
    internal fun bootstrap() {
        ...
        val properties =
            mutableMapOf(
                ...
                ProjectProperties.KOTLIN_DAEMON_ARGS_KEY to
        ...
        val regionMarkerPrefix = "# Begin: BB bootstrap-generated properties"
        val regionMarkerSuffix = "# End: BB bootstrap-generated properties"
        val prefix =
            """
      $regionMarkerPrefix
        ...
        if (mode == BootstrapPropertiesMode.LOG) {
            ...
            if (mode == BootstrapPropertiesMode.APPEND) {
                ...
                if (existingIndex != -1) {
                    ...
                    check(endIndex != -1) {
                        "Could not find region suffix for properties, please delete the BB properties below '$regionMarkerPrefix' and rerun."
                    }
    ...
    public companion object {
        ...
        public fun register(
            ...
            val bootstrap = project.tasks.register<CoreBootstrapTask>(NAME) {
                ...
                propertiesMode.setDisallowChanges(
                    project.providers
                        ...
                        .orElse(BootstrapPropertiesMode.APPEND)
                )
            }
...
internal object BootstrapUtils {
        ...
        companion object {
            fun fromProviders(
                ...
                return DaemonArgsProvider(
                    ...
                    minGradleXmx =
                    providers.gradleProperty(ProjectProperties.MIN_GRADLE_XMX).mapToInt().orElse(1),
    ...
    fun computeDaemonArgs(
        ...
        val deviceMemory = if (isCi) FileUtil.getLongFromFile("/sys/fs/cgroup/memory/memory.limit_in_bytes") else hardware.memory.total
        val memoryGb = (deviceMemory / bytesPerGB).roundToInt()
